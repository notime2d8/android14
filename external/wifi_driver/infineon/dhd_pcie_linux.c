 ...
 * Portions of this code are copyright (c) 2023 Infineon Corporation
 *
...
EXPORT_SYMBOL(dhdx_dongle_mem_dump_infineon);
#else
int
dhd_dongle_mem_dump_infineon(void)
{
	if (!g_dhd_bus) {
		DHD_ERROR(("%s: Bus is NULL\n", __FUNCTION__));
		return -ENODEV;
...
EXPORT_SYMBOL(dhd_dongle_mem_dump_infineon);
#endif /* BCMDHDX */
#endif /* DHD_FW_COREDUMP */

#ifdef BCMDHDX
bool
dhdx_bus_check_driver_up_infineon(void)
{
	dhd_bus_t *bus;
	dhd_pub_t *dhdp;
	bool isup = FALSE;
...
EXPORT_SYMBOL(dhdx_bus_check_driver_up_infineon);
#else
bool
dhd_bus_check_driver_up_infineon(void)
{
	dhd_bus_t *bus;
	dhd_pub_t *dhdp;
	bool isup = FALSE;
...
EXPORT_SYMBOL(dhd_bus_check_driver_up_infineon);
#endif /* BCMDHDX */
