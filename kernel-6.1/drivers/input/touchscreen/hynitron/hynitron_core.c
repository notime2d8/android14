...
#if HYN_EN_AUTO_UPDATE_WORKQUEUE
static void hyn_fw_update_func(struct work_struct *work);
#endif
/*****************************************************************************
...
static int hyn_irq_registration(struct i2c_client *client)
{
    ...
    if (node)
    {
	   ...
	   //  }
        ...
        ret = request_threaded_irq(hyn_ts_data->use_irq, NULL,hyn_eint_interrupt_handler, hyn_ts_data->pdata->irq_gpio_flags,
				"Hynitron Touch Int", hyn_ts_data);
        if (ret == 0 ){
		...
		HYN_ERROR("request_threaded_irq: %d", ret);
...
static void cst3xx_touch_report(struct work_struct *work)
{
	...
	if((hyn_ts_data->fw_updating >0)||(hyn_ts_data->work_mode == HYN_WORK_MODE_UPDATE)){
		HYN_ERROR(" fw_updating is running or un normalmode.\n");
		goto END;	
	}	
#endif
...
#if 0
{
	...
	HYN_DEBUG("gestrue_data:%x",buf[4]);

	if(0)
	{
			HYN_INFO("  HYN ENTER BIG_PALM MODE.\n");
			BIG_PALM_FLAG=1;
			input_report_key(hyn_ts_data->input_dev, KEY_POWER, 1);
		  input_sync(hyn_ts_data->input_dev);
		  input_report_key(hyn_ts_data->input_dev, KEY_POWER, 0);
		  input_sync(hyn_ts_data->input_dev);
			goto OUT_PROCESS;
	}
...
#if HYN_EN_AUTO_UPDATE
#if HYN_EN_AUTO_UPDATE_WORKQUEUE
	hyn_ts_data->hyn_fw_update_workqueue = create_singlethread_workqueue("hyn_fw_update_workqueue");
		if (!hyn_ts_data->hyn_fw_update_workqueue)
		HYN_ERROR(" hyn_fw_update_workqueue failed!");
	else{
		HYN_DEBUG(" hyn_fw_update_workqueue start.");
		INIT_DELAYED_WORK(&hyn_ts_data->hyn_fw_update_work, hyn_fw_update_func);
		mdelay(10);
		queue_delayed_work(hyn_ts_data->hyn_fw_update_workqueue, &hyn_ts_data->hyn_fw_update_work,5000);	
	}
#else
...
int hyn_input_dev_int(struct hynitron_ts_data *ts_data)
{
	...
	//set_bit(KEY_WAKEUP, hyn_ts_data->input_dev->keybit);
...
{	
	int ret = -1;
	HYN_DEBUG(" hyn_fw_update_func start.");
    ret = hyn_boot_update_fw(hyn_ts_data->client);
	if(ret<0){
		HYN_ERROR("hyn_boot_update_fw failed.\n");
	}
	if (hyn_ts_data->hyn_fw_update_workqueue){
    	destroy_workqueue(hyn_ts_data->hyn_fw_update_workqueue);
		hyn_ts_data->hyn_fw_update_workqueue = NULL;		
    }
}
#endif
...
static int hyn_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret<0) {
		...
		if(hyn_ts_data->pdata)  kfree(hyn_ts_data->pdata);
		kfree(ts_data);
		return ret;
	}
